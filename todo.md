...
- [ ] Step 4: lightweight non-moving mark/sweep GC. (table mark-bit support added; root scan now covers globals, value stack, current_env cells, and current_closure captured cells; sweep remains gated by gc_sweep_enabled while broader reachability work continues)
...
- [~] Add deferred reclaim staging for tables, then reclaim during GC sweep only. (ring queue + sweep helper emitted; op_pop staging wired; sweep gate + overflow diagnostics added; root-mark-aware table reclaim path wired across globals/vstack/current env/current closure)
